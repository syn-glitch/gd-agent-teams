<!--
 ...
 @file        COMMON_RULES.md
  ...
  @agent       꼼꼼이 (꼼꼼이 문서팀)
  @ordered-by  용남 대표
  @description 5개 에이전트 팀 공통 운영 규칙. ALC-Guardrail(이해-계획-완료) 3단계를 정의하며, 
               v2.2에서는 매뉴얼 중앙화 및 인덱스 관리 규칙을 추가한다.

  @change-summary
    AS-IS: v2.1 — Fast-Track, 자동화, 병렬 검수, 보안 스캔 필수화
    TO-BE: v2.2 — 매뉴얼 중앙화(manuals/ 집결) 및 INDEX.md 상시 동기화 규칙 추가

  @features
    - [추가] 규칙 0.4: 매뉴얼 중앙화 (모든 사용자 가이드는 manuals/ 폴더로 통합)
    - [추가] 규칙 0.5: INDEX.md 동기화 (신규 매뉴얼 생성 시 즉시 인덱스 업데이트)
    - [수정] 규칙 0.1~0.3 유지 및 ALC-Guardrail 보완
    - [추가] 규칙 6: 매뉴얼 및 사용자 가이드 관리 (<manuals_management>)

  ── 변경 이력 ──────────────────────────
  v2.2.0 | 2026-03-08 | 꼼꼼이 | 매뉴얼 중앙화 및 INDEX.md 업데이트 규칙 추가
  v2.1.0 | 2026-03-03 | 꼼꼼이 | Fast-Track, 자동화, 병렬 검수, 보안 스캔 강화
  v2.0.0 | 2026-03-02 | 꼼꼼이 | ALC-Guardrail 전사 강제 적용
...
---
- **문서 버전**: v2.2 (매뉴얼 거버넌스 강화)
- **작성일**: 2026-03-01
- **최종 수정일**: 2026-03-08
- **작성자**: 꼼꼼이 (Kkoomkkoom, Docs Team Lead)
- **검토자**: 용남 대표
- **상태**: approved
- **적용 대상**: 전사 에이전트 팀 (매뉴얼 통합 관리 프로세스 포함)
---
...
7. [규칙 6: 매뉴얼 및 사용자 가이드 관리](#규칙-6-매뉴얼-및-사용자-가이드-관리-manuals_management)
...
6. <manuals_management>  — 매뉴얼 저장 및 인덱스 관리 (v2.2 신설)
</common_rules_ref>
```
...
### 규칙 0: 에이전트 업무 생명주기 가드레일 (ALC-Guardrail)

모든 에이전트는 대표의 지시를 수행할 때 반드시 아래 **3단계 필수 절차**를 준수해야 한다.
이 절차를 어길 시 **[꼼꼼이 문서팀]의 집중 감사 대상**이 되며, 작업은 즉시 **중단**되어야 한다.

#### 1단계: [이해 보고] (Understanding Report) — 필수
대표님의 지시를 받자마자 **도구(Tool)를 사용하기 전**에 반드시 출력한다.
- **포함 항목**: `【원본 지시】`, `【의도 해석】`, `【작업 범위】`, `【산출물】`, `【확인 질문】`
- **목표**: 착수 전 대표님의 의도를 100% 동기화하여 자원 낭비를 방지한다.
- **규칙**: 하단에 **"승인해 주시면 [계획] 단계로 진입하겠습니다."** 명시 필수.
- **[v2.1 Fast-Track]**: 단순 오타 수정, UI 스타일링 등 저위험 작업은 이해 보고 후 승인 없이 2단계로 즉시 진입 가능 (단, 사후 보고 필수).

#### 2단계: [계획 보고] (Task Planning) — 필수
`task.md`를 생성하거나 업데이트한 후 반드시 출력한다.
- **포함 항목**: 업데이트된 `task.md` 내용 전체 또는 핵심 체크리스트.
- **목표**: 실제 실행 전 작업 로드맵을 확정받는다.
- **[v2.1 자동화]**: 에이전트는 도구 사용 시 `task.md` 상태를 자동으로 `[Auto-Update]` 하여 대표가 실시간 확인 가능하게 한다.
- **규칙**: 대표의 **명시적 승인** 없이 `write_file`, `clasp push` 등을 실행하는 것을 절대 금지한다. (단, Fast-Track 지시 시 예외)

#### 3단계: [완료 보고] (Completion Report) — 필수
모든 업무 완료 시 반드시 표준 양식으로 최종 보고한다.
- **포함 항목**: `【원본 지시】`, `【수행 결과 (체크리스트)】`, `【산출물 경로】`, `【위임 사항】`, `【토큰 사용량】`
- **[v2.1 보안]**: 완료 보고 전 반드시 **Secret Scanning(grep)**을 수행하고 "보안 스캔 통과"를 명시해야 한다.
- **목표**: 결과의 완결성을 보증하고 리소스 사용량을 투명하게 공유한다.

---

### 규칙 1: 태스크 프로토콜 (`<task_protocol>`)

ALC-Guardrail에 따라 아래 워크플로우를 반드시 따른다.

```markdown
━━━ STEP 1: 이해 보고서 출력 ━━━
- 지시 수신 즉시 출력 (도구 사용 금지)
- [v2.1] 오타/스타일링 등 단순 작업은 승인 대기 없이 STEP 2로 (Fast-Track)
- **[v2.3 Critical Rule]**: "간단한 작업"이라는 에이전트의 자의적 판단으로 STEP 1을 건너뛰는 것을 ★절대 금지★한다. (사례: 2026-03-10 워크플로우 위반 사고)

━━━ STEP 2: task.md 생성 및 계획 보고 ━━━
- task.md 작성 (GD_Agent_teams/[팀폴더]/tasks/YYYY-MM/task_[업무명].md)
- [v2.1] 도구 사용 시마다 `[Auto-Update]` 접두사로 진행 상황 로깅
- [v2.1] 개발 중인 문서는 `shared/` 폴더에 실시간 공유 (Side-car QA용)

━━━ STEP 3: 실행 및 실시간 검수 ━━━
- 대표 승인 후 체크리스트 순차 수행
- [v2.1] 타 팀장은 `shared/` 폴더 산출물을 읽고 병렬로 피드백(Side-car) 제공 가능
- 전체 완료 시 상태를 ✅ 완료로 변경한다
- 배포(Push) 전 반드시 `grep` 보안 스캔 수행
...
■ **[무조건 준수]**: 어떤 상황에서도 '이해 보고' 단계에서 대표님의 명시적 승인 후 작업을 진행한다. (Fast-Track 대상 제외)
...
## 규칙 6: 매뉴얼 및 사용자 가이드 관리 (`<manuals_management>`)

> **배경**: 여러 팀에서 각자 가이드 문서를 작성하다 보니, 사용자가 필요한 정보를 찾기 위해
> 모든 팀 폴더를 뒤져야 하는 불편함이 발생했다.
> 모든 사용자 대상 가이드는 한곳에 집결시키고 인덱스를 통해 관리한다.

```xml
<manuals_management>
■ 핵심 원칙
모든 사용자 대상 가이드(Manual, Guide, Tutorial 등)는 지정된 중앙 저장소에 저장하며,
통합 인덱스 파일을 통해 접근 가능해야 한다.

■ 상세 규칙
1. 저장 위치 통일:
   - 모든 사용자 가이드는 `GD_Agent_teams/kkoomkkoom-docs/manuals/` 폴더 내에 저장한다.
   - 팀별 폴더 내에 가이드를 남겨두지 않고, 반드시 위 경로로 이동 또는 생성한다.

2. 통합 인덱스(INDEX.md) 최신화:
   - 새로운 가이드 문서를 생성하거나 기존 문서를 수정/이동할 경우,
     즉시 `GD_Agent_teams/kkoomkkoom-docs/manuals/INDEX.md`의 리스트를 업데이트한다.
   - 인덱스 업데이트 시 문서 명칭, 주요 내용, 링크를 정확히 기재한다.

3. 네이밍 규칙 준수:
   - 파일명은 `[순번]_[소문자_주제명].md` 형식을 따른다. (예: 01_user_guide.md)
   - 이는 파일 정렬과 가독성을 높이기 위함이다.

4. 기존 경로 리다이렉트:
   - 기존 위치에서 파일을 이동한 경우, 원본 위치에 "이동 안내" 문구를 남겨
     사용자가 혼란을 겪지 않도록 한다.

■ 담당 및 위임
- 작성 주체: 각 가이드의 내용을 가장 잘 아는 담당 팀 (자비스, 강철 등)
- 관리/배치: 꼼꼼이 문서팀 (특히 **📚 아키비스트**)
- 절차:
  1. 담당 팀이 가이드 초안 작성
  2. 꼼꼼이 문서팀에 완료 보고 (위임 사항에 "매뉴얼 배치 및 인덱스 업데이트" 포함)
  3. 꼼꼼이 문서팀(아키비스트)이 `manuals/` 배치 및 `INDEX.md` 갱신
</manuals_management>
```
```
