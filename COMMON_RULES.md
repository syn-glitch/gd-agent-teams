<!--
 ...
 @file        COMMON_RULES.md
  @version     v2.1.0
  @updated     2026-03-03 00:05 (KST)
  @agent       꼼꼼이 (꼼꼼이 문서팀)
  @ordered-by  용남 대표
  @description 5개 에이전트 팀 공통 운영 규칙. ALC-Guardrail(이해-계획-완료) 3단계를 정의하며, 
               v2.1에서는 Fast-Track, 자동화, 병렬 검수 기능을 추가한다.

  @change-summary
    AS-IS: v2.0 — ALC-Guardrail 3단계 강제 적용 및 감사 조항
    TO-BE: v2.1 — Fast-Track(병목 해소), 자동화(Auto-Status), 병렬 검수(Side-car QA), 보안 스캔 필수화

  @features
    - [추가] 규칙 0.1: Fast-Track (리스크 낮은 작업의 즉시 실행 조건)
    - [추가] 규칙 0.2: Auto-Status (task.md 자동 업데이트 및 로깅 규격)
    - [추가] 규칙 0.3: Side-car QA (실시간 공유 폴더 shared/ 기반 병렬 검수)
    - [추가] 규칙 4.1: Pre-Push Secret Scan (배포 전 API 키 스캔 의무화)

  ── 변경 이력 ──────────────────────────
  v2.1.0 | 2026-03-03 00:05 | 꼼꼼이 | Fast-Track, 자동화, 병렬 검수, 보안 스캔 강화
  v2.0.0 | 2026-03-02 23:45 | 꼼꼼이 | ALC-Guardrail 전사 강제 적용
...
---
- **문서 버전**: v2.1 (Agile Guardrail 배포)
- **작성일**: 2026-03-01
- **최종 수정일**: 2026-03-03 00:05
- **작성자**: 꼼꼼이 (Kkoomkkoom, Docs Team Lead)
- **검토자**: 용남 대표
- **상태**: approved
- **적용 대상**: 전사 에이전트 팀 (Fast-Track 및 병렬 검수 적용)
---
