<!--
 ...
 @file        COMMON_RULES.md
  @version     v2.0.0
  @updated     2026-03-02 23:45 (KST)
  @agent       꼼꼼이 (꼼꼼이 문서팀)
  @ordered-by  용남 대표
  @description 5개 에이전트 팀 공통 운영 규칙. ALC-Guardrail(이해-계획-완료) 3단계를 정의한다.

  @change-summary
    AS-IS: v1.4 — 스타일 가이드 교정, 예외 규칙 신설
    TO-BE: v2.0 — ALC-Guardrail(업무 생명주기 가드레일) 3단계 강제 적용 및 감사 조항 신설

  @features
    - [신설] 규칙 0: ALC-Guardrail (업무 생명주기 가드레일)
    - [수정] 규칙 1: 태스크 프로토콜 (ALC-Guardrail 3단계에 맞춰 전면 개편)
    - [추가] 불이행 시 집중 감사 및 작업 중단 강력 패널티 명문화
...
---
- **문서 버전**: v2.0 (ALC-Guardrail 정식 배포)
- **작성일**: 2026-03-01
- **최종 수정일**: 2026-03-02 23:45
- **작성자**: 꼼꼼이 (Kkoomkkoom, Docs Team Lead)
- **검토자**: 용남 대표
- **상태**: approved
- **적용 대상**: 전사 에이전트 팀 (ALC-Guardrail 필수 적용)
---
...
- **규칙**: 대표의 **명시적 승인** 없이 `write_file`, `clasp push` 등을 실행하는 것을 절대 금지한다.
